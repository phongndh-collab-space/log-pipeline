...
export class LoggingMiddleware implements NestMiddleware {
  ...
  use(request: Request, response: Response, next: NextFunction) {
    ...
    if (xForwardedFor) {
      ...
      const rawRealIp = typeof xRealIp === 'string' ? xRealIp : Array.isArray(xRealIp) ? xRealIp[0] : '';
      client_ip = rawRealIp || request.ip || request.socket?.remoteAddress || '';
    }
    ...
    // Listen to response finish event to compute latency and log
    response.on('finish', () => {
